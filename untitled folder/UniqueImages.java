...
*/
    ...
    public static void main(String[] args) {
        ...
        //Compare images and remove duplicate images
        ArrayList<Image> imagesList = new ArrayList<Image>(set);
        for(int i = 0; i < imagesList.size(); i++) {
            for (int j = i + 1; j < imagesList.size(); j++) {
                if (imagesList.get(i).equals(imagesList.get(j))) {
                    imagesList.remove(imagesList.get(j));
                }
            }
        }
        
        System.out.println("Size of array list " + imagesList.size());
    }
}
